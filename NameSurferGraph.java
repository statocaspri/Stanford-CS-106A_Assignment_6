...
public class NameSurferGraph extends GCanvas
	...
	*/
	public void clear() {
		addEntryList.clear(); //clears the arraylist
	...
	public void update() {
		...
		GLabel word = new GLabel("1900");
			for (int i = 0; i < NDECADES; i++) {
				// add vertical lines and years in the bottom
		...
		add(new GLine(0, y1-GRAPH_MARGIN_SIZE, getWidth(), y1-GRAPH_MARGIN_SIZE));		// Bottom Horizontal Line
			...
			for (int i = 0; i < addEntryList.size(); i ++) {
				for (int j = 0; j < NDECADES; j++) {
					...
					// instruction says 0 is in bottom and not top
					if (z0 == 0) m = max;
					if (z1 == 0) n = max;
						
					if (j == NDECADES -1) {
						// skip that no line is drawn after last point
					} else {
